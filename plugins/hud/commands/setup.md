...
REGISTRY_EXISTS=$(grep -q "hud" ~/.claude/plugins/installed_plugins.json 2>/dev/null && echo "YES" || echo "NO")
...
1. Get plugin path:
   ```bash
   ls -td ~/.claude/plugins/cache/kellymears/hud/*/ 2>/dev/null | head -1
   ```
...
5. Generate command (quotes around runtime path handle spaces):
   ```
   bash -c '"{RUNTIME_PATH}" "$(ls -td ~/.claude/plugins/cache/kellymears/hud/*/ 2>/dev/null | head -1){SOURCE}"'
   ```
...
**Note**: The generated command dynamically finds and runs the latest installed plugin version. Updates are automatic - no need to re-run setup after plugin updates. If the HUD suddenly stops working, re-run `/hud:setup` to verify it's still installed.
...
3. **Common issues to check**:
   ...
   **"No such file or directory" for plugin**:
   - Plugin might not be installed: `ls ~/.claude/plugins/cache/kellymears/hud/`
   ...
   **WSL confusion**:
   ...
   - Check: `ls ~/.claude/plugins/cache/kellymears/hud/`
