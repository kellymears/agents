...
test('parseTranscript tolerates malformed lines', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'hud-'));
...
test('countConfigs honors project and global config locations', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'hud-home-'));
  const projectDir = await mkdtemp(path.join(tmpdir(), 'hud-project-'));
...
// https://github.com/jarrodwatts/hud/issues/3
