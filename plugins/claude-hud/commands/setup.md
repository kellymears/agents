...
CACHE_EXISTS=$(ls -d ~/.claude/plugins/cache/kellymears/claude-hud 2>/dev/null && echo "YES" || echo "NO")
...
**Windows (PowerShell)**:
```powershell
$cache = Test-Path "$env:USERPROFILE\.claude\plugins\cache\kellymears\claude-hud"
...
rm -rf ~/.claude/plugins/cache/kellymears/claude-hud
...
```powershell
# Remove orphaned cache
Remove-Item -Recurse -Force "$env:USERPROFILE\.claude\plugins\cache\kellymears\claude-hud" -ErrorAction SilentlyContinue
...
1. Get plugin path:
   ```bash
   ls -td ~/.claude/plugins/cache/kellymears/claude-hud/*/ 2>/dev/null | head -1
   ```
...
5. Generate command (quotes around runtime path handle spaces):
   ```
   bash -c '"{RUNTIME_PATH}" "$(ls -td ~/.claude/plugins/cache/kellymears/claude-hud/*/ 2>/dev/null | head -1){SOURCE}"'
   ```
...
1. Get plugin path:
   ```powershell
   (Get-ChildItem "$env:USERPROFILE\.claude\plugins\cache\kellymears\claude-hud" | Sort-Object LastWriteTime -Descending | Select-Object -First 1).FullName
   ```
...
4. Generate command (note: quotes around runtime path handle spaces in paths):
   ```
   powershell -Command "& {$p=(Get-ChildItem $env:USERPROFILE\.claude\plugins\cache\kellymears\claude-hud | Sort-Object LastWriteTime -Descending | Select-Object -First 1).FullName; & '{RUNTIME_PATH}' (Join-Path $p '{SOURCE}')}"
   ```
...
3. **Common issues to check**:
   ...
   **"No such file or directory" for plugin**:
   - Plugin might not be installed: `ls ~/.claude/plugins/cache/kellymears/claude-hud/`
   ...
   **WSL confusion**:
   ...
   - Check: `ls ~/.claude/plugins/cache/kellymears/claude-hud/`
