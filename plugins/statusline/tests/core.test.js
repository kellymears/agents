...
test('parseTranscript tolerates malformed lines', async () => {
  const dir = await mkdtemp(path.join(tmpdir(), 'statusline-'));
...
test('countConfigs honors project and global config locations', async () => {
  const homeDir = await mkdtemp(path.join(tmpdir(), 'statusline-home-'));
  const projectDir = await mkdtemp(path.join(tmpdir(), 'statusline-project-'));
...
// https://github.com/jarrodwatts/statusline/issues/3
