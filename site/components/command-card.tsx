...
export function CommandCard({ name, description, usage }: CommandCardProps) {
  return (
    <div className="glass-card p-5 my-6 group hover:shadow-glow-sm transition-all duration-300">
      <div className="flex items-center justify-between mb-3">
        <code className="text-lg font-mono font-semibold text-glass-400 group-hover:text-glow transition-all">
          {name}
        </code>
...
export function FeatureCard({ icon, title, description }: FeatureCardProps) {
  return (
    <div className="glass-card p-6 group hover:shadow-glow-sm transition-all duration-300">
      {icon && (
        <div className="w-10 h-10 rounded-xl bg-glass-500/10 border border-glass-500/20 flex items-center justify-center mb-4 text-glass-400 group-hover:shadow-glow-sm transition-all">
          {icon}
        </div>
      )}
